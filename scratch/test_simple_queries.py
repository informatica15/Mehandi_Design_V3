import requests
import json
from bs4 import BeautifulSoup

test_queries = {
    'bridal': 'bridal mehndi',
    'indo_arabic': 'indo arabic mehndi',
    'minimalist': 'minimalist mehndi',
    'floral': 'floral mehndi',
    'geometric': 'geometric mehndi',
    'rajasthani': 'rajasthani mehndi'
}

def test_query(category, query):
    url = f"https://www.bing.com/images/search?q={query}"
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
    }
    
    response = requests.get(url, headers=headers)
    soup = BeautifulSoup(response.text, 'html.parser')
    links = soup.find_all('a', class_='iusc')
    
    print(f"\nQuery: '{query}' -> Found {len(links)} links")
    for i, link in enumerate(links[:5]):
        try:
            m_attr = link.get('m')
            if m_attr:
                m_data = json.loads(m_attr)
                murl = m_data.get('murl')
                title = m_data.get('t') or 'No Title'
                print(f"  {i+1}: {title[:50]}... ({murl})")
        except Exception:
            continue

def main():
    for cat, query in test_queries.items():
        test_query(cat, query)

if __name__ == '__main__':
    main()
